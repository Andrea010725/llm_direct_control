...
class CarlaDataProvider(object):  # pylint: disable=too-many-public-methods
	...
	@staticmethod
	def create_blueprint(world, model, rolename='scenario', color=None, actor_category="car", attribute_filter=None):
		"""
		...
		try:
			# ipdb.set_trace()
			blueprints = world.get_blueprint_library().filter(model)
			...
			blueprint = CarlaDataProvider._rng.choice(world.get_blueprint_library().filter(bp_filter))
	...
	def request_new_actor(
			world, model, spawn_point, rolename='scenario', autopilot=False,
		...
		"""
		blueprint = CarlaDataProvider.create_blueprint(world, model, rolename, color, actor_category, attribute_filter)
		...
		if random_location:
			...
			actor = world.try_spawn_actor(blueprint, _spawn_point)

		if actor is None:
			if retry > 0:
				sleep(1.5)
				# print("RETRY-WARNING: Cannot spawn actor {} at position {}".format(model, spawn_point.location))
				return CarlaDataProvider.request_new_actor(world, model, spawn_point, rolename, autopilot, random_location, color, actor_category, attribute_filter, tick, retry - 1)
			else:
