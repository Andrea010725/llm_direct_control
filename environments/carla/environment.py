...
from environments.carla.sensors import Sensor, SensorSpecs

from environments.carla.navigation.behavior_agent import BehaviorAgent
from environments.carla.navigation import Route, RoutePlanner, RoadOption

from environments.carla.tools import misc, utils
from environments.carla.tools.utils import WAYPOINT_DICT
from environments.carla.tools.synchronous_mode import CARLASyncContext
...
class CARLABaseEnvironment(gym.Env):
    ...
    def __init__(self, address='localhost', port=2000, timeout=5.0, image_shape=(150, 200, 3), window_size=(800, 600),
        ...
        #self.fixed_delta_seconds=1.0 / fps
        #self.timeout = self.fixed_delta_seconds * 2
        ...
        self.world_settings = carla.WorldSettings(no_rendering_mode=False,
                                                  ...
                                                  fixed_delta_seconds= 0.01) #1.0 / fps)
        ...
        # Map
        self.current_town = 'Town03'  # loaded by default        #map修改
        ...
        self.origin_type = 'designation'  # map  'map' means sample a random point from the world's map
        self.origin = None
    ...
    def reset(self) -> dict:
        ...
        if self.should_spawn:
           self.spawn_actors_debug(self.origin_location, self.spawn_dict, self.selected_task)   #   这里进去生成 车和行人
           #  from rl.environments.carla.tools.scene_utils import SceneManager
           #  scenemanager = SceneManager(self.selected_task, self.world, self.client, self.vehicle)
           #  scenemanager.create_scene()
    ...
    def spawn_actors_debug(self,  origin_location: carla.Location, spawn_dict: dict, selected_task: str, hybrid=True, safe=True):
        """Instantiate vehicles and pedestrians in the current world"""
    ...
    def step(self, actions):
        ...
        next_state = self.get_observation(sensors_data)   #运行时长过长
    ...
    def close(self):
        ...
        if self.ego_cam:    #   解決warning問題
            self.ego_cam.destory()
    ...
    def reset_world(self):
        ...
        if self.origin_type == 'map':
            ...
            self.origin_location = carla.Location(x=self.origin.location.x, y=self.origin.location.y, z=self.origin.location.z)
            self.selected_task = self.choose_tasks_debug()   # DEBUG
        ...
        elif self.origin_type == 'designation':
            self.origin_point, self.selected_task = self.choose_tasks()
            # self.selected_task = 'GhostA'  # debug 用

            x = self.origin_point['x']
            y = self.origin_point['y']
            z = self.origin_point['z']
            yaw = self.origin_point['yaw']
            # debug 點   由於xml文件沒有更新 所以用 debug點判斷場景生成的是否正確
            # -78.08158874511719" y="-79.42774963378906" yaw="269.84375" z="-0.3572145104408264   这个点可以的
            # -71.98143768310547" y=" - 198.3694305419922" yaw=" - 219.49034118652344" z="0.5
            #x = -78.08158874511719
            #y = -79.42774963378906
            ##z = -0.3572145104408264
            #yaw = 269.84375
            self.origin_location = carla.Location(x=x, y=y, z=z)
            print("!!!!!origin_location!!!!!!", self.origin_location)
            self.origin = carla.Transform(location=carla.Location(x=x, y=y, z=z), rotation=carla.Rotation(pitch=0, yaw=yaw, roll=0))

        # choose destination (final point)
        if self.destination_type == 'map':
            self.destination = env_utils.random_spawn_point(self.map, different_from=self.origin).location
        ...
        if self.use_planner:
            self.route.plan(origin=self.origin.location, destination=self.destination)   #add location
    ...
    def choose_tasks(self):
        ...
        selected_task = 'ObstacleAhead'    #stage改这里
    ...
    def choose_tasks_debug(self):    # debug
        # 定义任务列表
        Tasks_list = [
                      'PrecedingVehicleStationary',
                      'FrontBrake',
...
class OneCameraCARLAEnvironment(CARLABaseEnvironment):
    ...
    # Surrounding vehicles: max_surrounding_vehicles * num_features_per_vehicle
    SURROUNDING_FEATURES = dict(space=spaces.Box(low=-np.inf, high=np.inf, shape=(10 * 5,)),
                                default=np.zeros(shape=(10 * 5,), dtype=np.float32))
    ...
    def __init__(self, *args, disable_reverse=False, min_throttle=0.0, camera='segmentation',
        ...
        # print("!!!! OneCameraCARLAEnvironment(CARLABaseEnvironment) !!!!")
...
class OneCameraCARLAEnvironmentDiscrete(OneCameraCARLAEnvironment):
    ...
    def __init__(self, bins: int, *args, **kwargs):
        ...
        super().__init__(*args, **kwargs)
        # print("!!!! Use OneCameraCARLAEnvironmentDiscrete(OneCameraCARLAEnvironment) !!!!")
...
class ThreeCameraCARLAEnvironment(OneCameraCARLAEnvironment):
    ...
    def __init__(self, *args, image_shape=(120, 160, 1), window_size=(600, 300), **kwargs):
        ...
        # print("!!!! Use ThreeCameraCARLAEnvironment(OneCameraCARLAEnvironment) !!!!")
...
class CARLABenchmark(CARLAWrapper):
    ...
    def __init__(self, env: CARLABaseEnvironment, task: Tasks, preset='test', weather=None, avg_speed=10.0):
        ...
        # self.env.spawn_actors(spawn_dict=self.task_spec)     debug
