...
sys.path.append("D:\Github仓库\llm_direct_control\environments\carla")
...
sys.path.append("D:\CARLA_0.9.14\WindowsNoEditor\PythonAPI\carla\dist\carla-0.9.10-py3.7-linux-x86_64.egg")
...
def spawn_actor(world: carla.World, blueprint: carla.ActorBlueprint, spawn_point: carla.Transform,
    ...
    """
    actor = world.try_spawn_actor(blueprint, spawn_point, attach_to, attachment_type)     # 这一行生成的是Nonetype
...
class FrontBrake:
    def __init__(self, world, ego_wp):
        self.world = world
        
        # 生成前车的位置（在自车前方20米）
        front_transform = ego_wp.next(20)[0].transform
        front_transform.location.z += 0.5
        
        # 生成前车
        blueprint = random_blueprint(world, actor_filter='vehicle.*')
        self.front_vehicle = spawn_actor(world, blueprint, front_transform)
        
        # 初始化计数器和状态
        self.tick_counter = 0
        self.distance_traveled = 0
        self.initial_location = self.front_vehicle.get_location()
        self.is_moving = True
        
        # 初始化位置记录列表
        self.front_vehicle_positions = []
        
    def tick(self):
        """
        每个tick调用此方法来更新前车状态并记录位置信息
        """
        if self.is_moving:
            # 前车正常行驶
            self.front_vehicle.apply_control(carla.VehicleControl(throttle=0.3))
            
            # 计算已行驶距离
            current_location = self.front_vehicle.get_location()
            self.distance_traveled = current_location.distance(self.initial_location)
            
            # 如果行驶距离超过20米，停车
            if self.distance_traveled >= 20:
                self.is_moving = False
                self.front_vehicle.apply_control(carla.VehicleControl(throttle=0, brake=1.0))
    ...
    def record_position(self):
        ...
        try:
            # 获取当前位置和速度
            current_location = self.front_vehicle.get_location()
            current_velocity = self.front_vehicle.get_velocity()
            timestamp = time.time()
            ...
            # 追加新的位置数据
            self.front_vehicle_positions.append(position_data)
    ...
    def save_positions(self):
        ...
        """
        try:
            with open('scene_data/front_vehicle_positions.json', 'w', encoding='utf-8') as f:
                json.dump(self.front_vehicle_positions, f, indent=4, ensure_ascii=False)
        except Exception as e:
            print(f"保存前车位置数据时出错: {e}")
...
def FrontBrake(client, world: carla.World, ego_wp):
    gen_traffic_flow(client, world, ego_wp)
...
def GiveAwayEmergencyRight(world: carla.World, ego_wp):    # 先不放了吧 没有后视摄像头
    gen_traffic_flow( world, ego_wp)
...
def ObstacleAhead(client, world: carla.World, ego_wp):
    gen_traffic_flow(client,world, ego_wp)
    obstacle_ahead(world, ego_wp)
    return
