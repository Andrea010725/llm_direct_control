...
class TCP_planner(pl.LightningModule):
	def __init__(self, config, lr):
		...
		# ckpt = torch.load("/home/wupenghao/transfuser/selfD/log/lb_ytb/best_epoch=13-val_loss=0.429.ckpt",map_location='cpu')['state_dict']
		...
		# ACO
		# ckpt = torch.load("/home/wupenghao/transfuser/ACO/log/ACO_paper/aco_pjlab.ckpt",map_location='cpu')
		# self.model.perception.load_state_dict(ckpt, strict = False)


		# ckpt = torch.load("/home/wupenghao/transfuser/monodepth2/log/ytb_full_predK_singlepose_jointfinetune_aug/best_epoch=59-val_loss=0.055.ckpt",map_location='cpu')['state_dict']

		# new_state_dict = OrderedDict()
		# for key, value in ckpt.items():
		# 	# new_key = key.replace("model.perception.","")
		# 	new_key = key.replace("posenet.pose_encoder.encoder.","")
		# 	new_state_dict[new_key] = value
		# self.model.perception.load_state_dict(new_state_dict, strict = False)
		...
		# moco
		# ckpt = torch.load("/home/wupenghao/transfuser/ACO/ACO/code/log_moco/checkpoint_0099.pth.tar",map_location='cpu')['state_dict']
		# new_state_dict = OrderedDict()
		# for key, value in ckpt.items():
		# 	if 'encoder_k' in key:
		# 		continue
		...
		#MIM
		# ckpt = torch.load("/home/wupenghao/transfuser/SimMIM/resnet34_100ep_32_ytb/simmim_pretrain/simmim_pretrain__resnet34__img256_896_window6__100ep/ckpt_epoch_99.pth",map_location='cpu')['model']
		# new_state_dict = OrderedDict()
		# for key, value in ckpt.items():
		...
		# for param in self.model.perception.parameters():
		# 	param.requires_grad = False

		# self.model.perception.eval()
	...
	def configure_optimizers(self):
		...
		optimizer = optim.Adam(filter(lambda p: p.requires_grad, self.parameters()), lr=self.lr, weight_decay=1e-7)
...
if __name__ == "__main__":
	...
	parser.add_argument('--id', type=str, default='lb2_90routes_rgbhigh_half', help='Unique experiment identifier.')
