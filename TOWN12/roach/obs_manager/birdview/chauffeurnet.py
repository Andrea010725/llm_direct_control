...
class ObsManager():
	def __init__(self, obs_configs, criteria_stop=None):
		...
		# self._width = int(obs_configs['width_in_pixels']/10)
	...
	def attach_ego_vehicle(self, ego_vehicle):
		...
		world_name = self._world.get_map().name
		
		maps_h5_path = self._map_dir / (world_name[-6:]+ '.h5')
		with h5py.File(maps_h5_path, 'r', libver='latest', swmr=True) as hf:
			...
			self._world_offset_origin = np.array(hf.attrs['world_offset_in_meters'], dtype=np.float32)
			self._world_width_origin = int(hf.attrs['width_in_pixels'])
			if '12' in world_name:
				self._world_width = int(hf.attrs['width_in_pixels']/10)
			else:
				self._world_width = int(hf.attrs['width_in_pixels'])
	...
	def get_observation(self, route_plan):
		...
		def get_crop_loc(ev_loc_in_px):
			x_min = int(ev_loc_in_px[0] - self._world_width//2)
			x_max = int(x_min + self._world_width)
			y_min = int(ev_loc_in_px[1] - self._world_width//2)
			y_max = int(y_min + self._world_width)
			if x_min < 0:
				x_min = 0
				x_max = x_min + (self._world_width-1)
			if x_max > self._world_width_origin-1:
				x_max = self._world_width_origin-1
				x_min = x_max - (self._world_width-1)
			if y_min < 0:
				y_min = 0
				y_max = y_min + (self._world_width-1)
			if y_max > self._world_width_origin-1:
				y_max = self._world_width_origin-1
				y_min = y_max - (self._world_width-1)
			return x_min, x_max, y_min, y_max

		ev_loc_in_px = self._world_to_pixel_origin(ev_loc)
		x_min, x_max, y_min, y_max = get_crop_loc(ev_loc_in_px)
		crop_road = self._road[y_min:y_max, x_min:x_max]
		crop_lane_marking_all = self._lane_marking_all[y_min:y_max, x_min:x_max]
		crop_lane_marking_white_broken = self._lane_marking_white_broken[y_min:y_max, x_min:x_max]

		self._world_offset[0] = self._world_offset_origin[0] + x_min/self._pixels_per_meter
		self._world_offset[1] = self._world_offset_origin[1] + y_min/self._pixels_per_meter
		

		parked_list_bbx = CarlaDataProvider._parked_vehicles
		vehicle_bbox_list = self._world.get_level_bbs(carla.CityObjectLabel.Vehicles)
		vehicle_bbox_list = filter(lambda x: x not in parked_list_bbx, vehicle_bbox_list)
		...
		# road_mask, lane_mask
		road_mask = cv.warpAffine(crop_road, M_warp, (self._width, self._width)).astype(np.bool)
		lane_mask_all = cv.warpAffine(crop_lane_marking_all, M_warp, (self._width, self._width)).astype(np.bool)
		lane_mask_broken = cv.warpAffine(crop_lane_marking_white_broken, M_warp,
	...
	def _world_to_pixel_origin(self, location, projective=False):
		"""Converts the world coordinates to pixel coordinates"""
		x = self._pixels_per_meter * (location.x - self._world_offset_origin[0])
		y = self._pixels_per_meter * (location.y - self._world_offset_origin[1])
