...
class CARLA_Data(Dataset):
	...
	def __init__(self, root, data_folders, img_aug = False):
		...
		if img_aug:
			self.front_img = self.front_img[::2]
			self.x = self.x[::2]
			self.y = self.y[::2]
			...
			self.action_mu = self.action_mu[::2]
			self.action_sigma = self.action_sigma[::2]


			self.x_command = self.x_command[::2]
			self.y_command = self.y_command[::2]
			self.only_ap_brake = self.only_ap_brake[::2]


			self.future_x = self.future_x[::2]
			self.future_y = self.future_y[::2]
			self.future_theta = self.future_theta[::2]

			self.future_feature = self.future_feature[::2]
			self.future_action = self.future_action[::2]
			self.future_action_mu = self.future_action_mu[::2]
			self.future_action_sigma = self.future_action_sigma[::2]
			self.future_only_ap_brake = self.future_only_ap_brake[::2]
	...
	def __getitem__(self, index):
		...
		self.front_img[index][0] = self.front_img[index][0].replace('rgb', 'rgb_high')
		if self.img_aug:
			data['front_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(
					scale_and_crop_image(Image.open(self.root+self.front_img[index][0]), crop_w=800, crop_h=384))))
		else:
		...
		# if self.img_aug:
		# 	data['front_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(
		# 			scale_and_crop_image(Image.open(self.root+self.front_img[index][0]), crop_w=900, crop_h=256))))
		# else:
		# 	data['front_img'] = self._im_transform(np.array(
		# 			scale_and_crop_image(Image.open(self.root+self.front_img[index][0]), crop_w=900, crop_h=256)))
...
# root_lb2 = '/home/wupenghao/transfuser/data_roach_90/'
# for i in range(8):
# 	train_data.append(os.path.join(root_lb2, 'roach_ap_90routes_{}'.format(i)))
# train_set = CARLA_Data(root = '/home/wupenghao/transfuser/data_roach_90/', data_folders =train_data, img_aug = False)
