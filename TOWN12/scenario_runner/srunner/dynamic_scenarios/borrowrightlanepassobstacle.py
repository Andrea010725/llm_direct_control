...
class BorrowRightLanePassObstacle(BasicScenario):
    """
    Desc: TODO
    Special: 补充TODO的数据
    """
    ...
    def __init__(self, world, ego_vehicles, config, randomize=False, debug_mode=False, criteria_enable=True, timeout=180, uniad=False, interfuser=False):
        ...
        self.stages = {
            '#fix1': (KEEP_L, [], KEEP_S, [], ''),
            '#dynamic1': (RIGHT_B, [], DEC, [], ''),
            '#dynamic2': (RIGHT_B, [], KEEP_S, [], ''),
            '#fix2': (RIGHT_B, [], KEEP_S, [], ''),
            '#dynamic3': (RIGHT_B, [], KEEP_S, [], ''),
            '#fix3': (KEEP_L, [], ACC, [], ''),
            '#dynamic4': (RIGHT_B, [], KEEP_S, [], ''),
            '#fix4': (RIGHT_B, [], KEEP_S, [], ''),
            '#dynamic5': (RIGHT_B, [], KEEP_S, [], ''),
            '#fix5': (KEEP_L, [], ACC, [], ''),
            '#fix6': (KEEP_L, [], KEEP_S, [], ''),
        }

        super().__init__("BorrowRightLanePassObstacle", ego_vehicles, config, world, randomize, debug_mode, criteria_enable=criteria_enable, uniad=uniad, interfuser=interfuser)
            ...
            if self.stage == 0:
                ...
                if cur_wp.lane_id == front_obstacle_wp.lane_id:
                    if distance > 30:
                        ...
                        ego_stage = '#dynamic1'
                        ego_action = 'decelerate|borrow|lanechange|right|prepare'
                        left_car_num = sum([1 for item in self.actor_desc if 'left' in item])
                        if left_car_num > 0:
                            ego_reason = '前方障碍物&左侧有多辆车'
                            explainable_desc = f'{desc.direction}{desc.distance}处有{desc.type}，同时左侧有{left_car_num}辆车，减速并准备向右借道'
                        else:
                            ego_reason = '前方障碍物'
                            explainable_desc = f'{desc.direction}{desc.distance}处有{desc.type}，减速并准备向右借道'
                        ...
                        ego_stage = '#dynamic2'
                        ego_action = 'keep|borrow|lanechange|right|stage1'
                        left_car_num = sum([1 for item in self.actor_desc if 'left' in item])
                        if left_car_num > 0:
                            ego_reason = '前方障碍物&左侧有多辆车'
                            explainable_desc = f'{desc.direction}{desc.distance}处有{desc.type}，同时左侧有{left_car_num}辆车，向右借道'
                        else:
                            ego_reason = '前方障碍物'
                            explainable_desc = f'{desc.direction}{desc.distance}处有{desc.type}，向右借道'
                    else:
                        print(f'Error: distance = {distance}m')
                        raise NotImplementedError
                else:
                    if cur_ego_loc.distance(cur_wp.transform.location) < cur_wp.lane_width / 4.0:
                        # Desc: 完成变道
                        ego_stage = '#fix2'
                        ego_action = 'keep|borrow|right|stage2'
                        ego_reason = '左侧障碍物'
            ...
            # Special: 第二阶段：借道中
                ...
                if cur_wp.lane_id == front_obstacle_wp.lane_id:
                    if cur_ego_loc.distance(cur_wp.transform.location) < cur_wp.lane_width / 4.0:
                        ...
                        # Desc: 还没完成返回原车道
                        ego_stage = '#dynamic4'
                        ego_action = 'keep|borrow|lanechange|left'
                        ego_reason = '左侧障碍物'
                        explainable_desc = f'已通过{desc.type}，向左变道，完成借道中'
                else:
                    # Desc: 还在右侧车道
                    if distance > 8 and actor_front_of_ego(data) is False and self.lane_changed is False:
    ...
    def _initialize_actors(self, config):
        ...
        if lane_info.l2r > 1 and random.random() < 0.9:
            right_vehicles = left_lane_vehicle_flow_scenario(self.starting_wp)
    ...
    def _create_behavior(self):
        root = py_trees.composites.Parallel(name="BorrowRightLanePassObstacle", policy=py_trees.common.ParallelPolicy.SUCCESS_ON_ONE)
        c1 = py_trees.composites.Sequence(name="BorrowRightLanePassObstacle_c1")
