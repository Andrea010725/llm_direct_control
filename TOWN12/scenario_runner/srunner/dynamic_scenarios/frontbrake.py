...
from TOWN12.scenario_runner.srunner.scenariomanager.scenarioatomics.atomic_trigger_conditions import (DriveDistance, WaitEndIntersection, InTriggerDistanceToVehicle, InTriggerDistanceToNextIntersection, StandStill)
...
class FrontBrake(BasicScenario):
    ...
    def interfuser_tick_autopilot(self):
        ...
        # # 检查是否已经过了60秒
        if current_time - last_run_time >= 6:
            # 更新上次运行的时间
    ...
    def _initialize_actors(self, config):
        ...
        # For: 右侧交通流
        right_traffic_flow_scenario(self.starting_wp, self.other_actors, self.actor_desc, scene_cfg={'filters': '+hcy1', 'idp': 0.7}, gen_cfg={'name_prefix': 'right'})
        # For: 左侧交通流
        left_traffic_flow_scenario(self.starting_wp, self.other_actors, self.actor_desc, scene_cfg={'filters': '+hcy1', 'idp': 0.8}, gen_cfg={'name_prefix': 'left'})
        # For: 对向交通流
        opposite_traffic_flow_scenario(self.starting_wp, self.other_actors, self.actor_desc, scene_cfg={'filters': '+hcy1', 'idp': 0.7, 'backward_num': 15}, gen_cfg={'name_prefix': 'opposite'})
        # For: 路边停靠车辆
        right_parking_vehicle_scenario(self.starting_wp, self.other_actors, self.actor_desc, scene_cfg={'filters': '+wheel4-large', 'idp': 0.8, 'forward_num': 15}, gen_cfg={'name_prefix': 'park'})
    ...
    def _create_behavior(self):
        ...
        c1.add_child(DriveDistance(self.other_actors[self.front_index], 10))
        c1.add_child(DriveDistance(self.ego_vehicles[0], 3))
        c1.add_child(StandStill(self.other_actors[self.front_index], name="StandStill", duration=2))
        c1.add_child(StandStill(self.ego_vehicles[0], name="StandStill", duration=2))
        root.add_child(c1)
