...
from aim_v2.model import AIM_V2
from aim_v2.config import GlobalConfig
...
class AIMV2Agent(autonomous_agent_local.AutonomousAgent):
	...
	def tick(self, input_data):
		...
		# local_command_point = np.array([next_wp[1]-pos[1], -1*(next_wp[0]-pos[0]), 0])
		# local_command_point = vec_global_to_ref(local_command_point, {'yaw':np.rad2deg(compass)-90, 'pitch':0, 'roll':0})
		# result['target_point'] = tuple(local_command_point[:2])
	...
	def run_step(self, input_data, timestamp):
		...
		speed = torch.FloatTensor([tick_data['speed']]).view(1,1).to('cuda', dtype=torch.float32)
		speed = speed / 12
		...
		pred_wp = self.net(rgb, state, target_point)["pred_wp"]

		steer, throttle, brake, metadata = self.net.control_pid_neat(pred_wp, gt_velocity, target_point)
		...
		if tick_data['speed'] > 0.1:
			self.last_moving_status = self.status
