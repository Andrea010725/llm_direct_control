...
from nav_planner import PIDController, RoutePlanner, interpolate_trajectory, _location_to_gps, _get_latlon_ref
...
class AutoPilot(autonomous_agent_local.AutonomousAgent):
	def setup(self, path_to_conf_file, route_index=None, route_name=None):
		...
		if SAVE_PATH is not None:
			...
			(self.save_path / 'rgb_high').mkdir(parents=True, exist_ok=True)
			(self.save_path / 'rgb_pitch').mkdir(parents=True, exist_ok=True)
	...
	def _init(self, hd_map):
		...
		lat_ref, lon_ref = _get_latlon_ref(self.world_map)
	...
	def _get_control(self, input_data, steer=None, throttle=None,
		...
		if static_signal:
			...
			same_lane = bool(self.world_map.get_waypoint(static_signal.get_location(), project_to_road=True, lane_type=carla.LaneType.Any).road_id == ego_vehicle_waypoint.road_id)
			
			if not same_lane:
				if((self.step % self.save_freq == 0) and (self.save_path is not None)):
					self.save_info(distance, same_lane, pos, None, None, None, None)
			else:
				...
				if lane_wp is not None:
					def get_lane_wp():
						...
						return trajectory		
					planned_route = [[item.x, -1*item.y] for item in get_lane_wp()]
					
				else:
					planned_route = None
				direction = CarlaDataProvider._accident_direction
				if((self.step % self.save_freq == 0) and (self.save_path is not None)):
					self.save_info(distance, same_lane, pos, lane_change, ready_to_change_lane, planned_route, direction)

				if distance < 8:
					self.flag = True

				if self.flag and distance > 15:
					x = y
	...
	def save_info(self, distance, same_lane, pos, lane_change, ready_to_change_lane, planned_route, direction):
		frame = self.step // self.save_freq

		data = {
				'distance': distance,
				'same_lane': same_lane,
				'pos_x':float(self._vehicle.get_location().x),
				'pos_y':float(-1 * self._vehicle.get_location().y),
				'yaw':float(self._vehicle.get_transform().rotation.yaw/180*np.pi + np.pi/2),
				'lane_change': lane_change,
				'ready_to_change_lane': ready_to_change_lane,
				'planned_route': np.array(planned_route).tolist(),
				'direction': direction,
				}

		info_file = self.save_path / 'info' / ('%04d.json' % frame)
