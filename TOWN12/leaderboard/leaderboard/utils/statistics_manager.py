...
from TOWN12.scenario_runner.srunner.scenariomanager.traffic_events import TrafficEventType

from TOWN12.leaderboard.leaderboard.utils.checkpoint_tools import fetch_dict, save_dict
...
PENALTY_VALUE_DICT = {
	...
	TrafficEventType.STOP_INFRACTION: 1.0,
...
class StatisticsManager(object):
	...
	def save_progress(self, route_index, total_routes):
		self._results.checkpoint.progress = [route_index+1, total_routes]
	...
	def compute_route_statistics(self, config, duration_time_system=-1, duration_time_game=-1, failure_message=""):
		...
		index = config.index
		route_record = self._results.checkpoint.records[0] # WENYANG: TOWN12 VERSION [CHANGE 'index' WITH 0]
	...
	def compute_global_statistics(self):
		...
		for route_record in route_records:
			self._total_routes = 1 # WENYANG FIX for TOWN12
