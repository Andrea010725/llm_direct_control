...
from TOWN12.scenario_runner.srunner.scenarioconfigs.scenario_configuration import ActorConfigurationData
from TOWN12.scenario_runner.srunner.scenariomanager.carla_data_provider import CarlaDataProvider

from TOWN12.scenario_runner.srunner.scenariomanager.scenarioatomics.atomic_behaviors import ScenarioTriggerer, Idle
from TOWN12.scenario_runner.srunner.scenariomanager.scenarioatomics.atomic_trigger_conditions import WaitForBlackboardVariable
...
from TOWN12.scenario_runner.srunner.scenarios.background_activity import BackgroundBehavior
from TOWN12.scenario_runner.srunner.scenariomanager.weather_sim import RouteWeatherBehavior
from TOWN12.scenario_runner.srunner.scenariomanager.lights_sim import RouteLightsBehavior
from TOWN12.scenario_runner.srunner.scenariomanager.timer import RouteTimeoutBehavior

from TOWN12.leaderboard.leaderboard.utils.route_parser import RouteParser, DIST_THRESHOLD
from TOWN12.leaderboard.leaderboard.utils.route_manipulation import interpolate_trajectory

import TOWN12.leaderboard.leaderboard.utils.parked_vehicles as parked_vehicles

import re
def name_convert_to_camel(name: str) -> str:
    s = re.sub(r'(_[a-z])', lambda x: x.group(1)[1].upper(), name)
    s = list(s)
    s[0] =s[0].upper()
    return ''.join(s)

EVAL = os.environ.get('EVAL', None)
IGNORE_SCENARIO = os.environ.get('IGNORE_SCENARIO', None)

filtered_scenarios = ['ParkingExit', 
                      'ConstructionObstacle', 'ConstructionObstacleTwoWays', 'Accident', 'AccidentTwoWays', 'ParkedObstacle', 'ParkedObstacleTwoWays', 'VehicleOpensDoorTwoWays', 'HazardAtSideLane', 'HazardAtSideLaneTwoWays', 'InvadingTurn',
...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        if debug_mode>0:
            self._draw_waypoints(world, self.route, vertical_shift=0.1, size=0.02, persistency=10000)
    ...
    def _get_route(self, config):
        ...
        self.gps_route, self.route, self.wp_route = interpolate_trajectory(config.keypoints)
        return self.route
    ...
    def _spawn_parked_vehicles(self):
        ...
        for slot in all_available_parking_slots:
            ...
            if is_free:
                ...
                pass
        prev_list = (CarlaDataProvider.get_world().get_level_bbs(carla.CityObjectLabel.Vehicles))
        CarlaDataProvider.get_client().apply_batch_sync(batch)
        after_list = (CarlaDataProvider.get_world().get_level_bbs(carla.CityObjectLabel.Vehicles))
        new_list = []
        for _ in after_list:
            if _ not in prev_list:
                new_list.append(_)
        CarlaDataProvider._parked_vehicles = new_list
    ...
    def get_all_scenario_classes(self):
        ...
        for scenario_file in scenarios_list:
            ...
            for member in inspect.getmembers(scenario_module, inspect.isclass):
                ...
                all_scenario_classes[name_convert_to_camel(member[0])] = member[1]
    ...
    def _build_scenarios(self, world, ego_vehicle, scenario_definitions, scenarios_per_tick=5, timeout=300, debug=False):
        ...
        if debug:
            ...
            for scenario_config in scenario_definitions:
                if IGNORE_SCENARIO:
                    if scenario_config.type not in remaining_scenarios:
                        continue
                ...
                world.debug.draw_point(debug_loc, size=0.1, color=carla.Color(128, 0, 0), life_time=1000000)
                # world.debug.draw_string(debug_loc, str(scenario_config.name), draw_shadow=False,
                #                         color=carla.Color(128, 0, 0), life_time=1000000, persistent_lines=True)

        for scenario_number, scenario_config in enumerate(scenario_definitions):
            if IGNORE_SCENARIO:
                if scenario_config.type not in remaining_scenarios:
                    print('ignore scenario:', scenario_config.type)
                    continue
    ...
    def _create_test_criteria(self):
        ...
        if EVAL:
            criteria.add_child(CollisionTest(self.ego_vehicles[0], name="CollisionTest", terminate_on_failure=False))
        else:
            criteria.add_child(CollisionTest(self.ego_vehicles[0], name="CollisionTest", terminate_on_failure=True))
        if EVAL:
            criteria.add_child(RunningRedLightTest(self.ego_vehicles[0], terminate_on_failure=False))
        else:
            criteria.add_child(RunningRedLightTest(self.ego_vehicles[0], terminate_on_failure=True))
        ...
        criteria.add_child(ActorBlockedTest(
            self.ego_vehicles[0], min_speed=0.1, max_time=120.0, terminate_on_failure=True, name="AgentBlockedTest")
        )
